...
module.exports = {
  ...
  stash(sourceCode, options) {
    this.commentsCache = {}
    let commentsIndex = 0
    sourceCode = sourceCode.replace(/(\/\/.*)|(\/\*[\s\S]*?\*\/)/g, (match, comment1, comment2, offset, string) => {
      let comment = ''
      ...
      if (comment) {
        ...
        this.commentsCache[commentsKey] = comment
        return match.replace(comment, commentsKey)
      } else {
        return match
      }
    })
