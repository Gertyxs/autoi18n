...
module.exports = {
  ...
  /**
   * 获取嵌套的html标签 可以 返回多段 主要用来处理 react jsx 模板
   * @param options.code 需要匹配html的字符串
   * @param options.tagName 需要匹配html 标签名称 如果传 默认匹配所有标签
   */
  getNestedTags({ code, tagName }) {
    tagName = tagName ? tagName : '\\w*'
    // 标签编号对象
    let tagsNo = {}
    // 开始标签编号
    code = code.replace(new RegExp(`(<(${tagName})((?:\\s+[^>]*)|)>)|(<\\/(${tagName})>)|(<(${tagName})((?:\\s+[^>]*)|)\\/>)`, 'gim'), (match, startTag, starTagName, startTagCon, endTag, endTagName, closeTag, closeTagName, closeTagCon) => {
      // 如果是闭合标签  <img /> 类似这种
      let value = ''
      if (closeTag) {
    ...
    // 编号完成 进行匹配
    let match = code.match(new RegExp(`<(${tagName}%%_0)((?:\\s+[^>]*)|)>[\\s\\S]*?<\\/\\1>|<(${tagName}%%_0)((?:\\s+[^>]*)|)\\/>`, 'gim')) || []
    // 编号完成清除标签的编号
    for (var i = 0; i < match.length; i++) {
      match[i] = match[i].replace(new RegExp(`(<(${tagName}%%_\\-?\\d+)((?:\\s+[^>]*)|)\\/?>)|(<\\/(${tagName}%%_\\-?\\d+)>)`, 'gim'), (match) => {
